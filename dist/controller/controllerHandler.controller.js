...
export const createAndAssignController = controllerWrapper(async (req, res) => {
    ...
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Contrôleur créé et affecté", data: assignment }));
});
/** Affecter un contrôleur existant (par email) à un événement */
export const assignExistingController = controllerWrapper(async (req, res) => {
    const organizerId = req.user.userId;
    const { eventId } = req.params;
    ...
    const assignment = await eventControllerRepository.assign(eventId, controller.id);
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Contrôleur affecté", data: assignment }));
});
/** Lister les contrôleurs d'un événement */
export const listEventControllers = controllerWrapper(async (req, res) => {
    const organizerId = req.user.userId;
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Contrôleurs récupérés", data: controllers }));
});
/** Retirer un contrôleur d'un événement */
export const removeController = controllerWrapper(async (req, res) => {
    const organizerId = req.user.userId;
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Contrôleur retiré" }));
});
// ── Contrôleur : son propre dashboard ────────────────────────────────────────
/** Événements assignés au contrôleur connecté */
export const getMyAssignedEvents = controllerWrapper(async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Événements assignés récupérés", data: events }));
});
/** Historique des scans du contrôleur connecté */
export const getMyScanHistory = controllerWrapper(async (req, res) => {
    const controllerId = req.user.userId;
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Historique récupéré", data: tickets }));
});
/** Vérifier un billet (contrôleur) — uniquement pour ses événements assignés */
export const verifyTicketAsController = controllerWrapper(async (req, res) => {
    const controllerId = req.user.userId;
    const { qrData } = req.body;
    if (!qrData)
        throw new AppError("qrData requis", StatusCodes.BAD_REQUEST);
    const ticket = await ticketRepository.findByQrData(qrData);
    if (!ticket)
        throw new AppError("Billet invalide ou introuvable", StatusCodes.NOT_FOUND);
    ...
    if (ticket.status === "used")
        throw new AppError("Ce billet a déjà été utilisé", StatusCodes.BAD_REQUEST);
    if (ticket.status === "expired")
        throw new AppError("Ce billet est expiré", StatusCodes.BAD_REQUEST);
    if (ticket.status !== "valid")
        throw new AppError("Ce billet n'est pas valide", StatusCodes.BAD_REQUEST);
    const updated = await prisma.ticket.update({
        where: { id: ticket.id },
        data: { status: "used", usedAt: new Date(), scannedById: controllerId },
    });
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Billet validé avec succès", data: updated }));
});
//# sourceMappingURL=controllerHandler.controller.js.map