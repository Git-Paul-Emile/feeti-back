...
export const getAllCountries = controllerWrapper(async (_req, res) => {
...
export const createCountry = controllerWrapper(async (req, res) => {
    const { code, name, flag } = req.body;
    if (!code || !name)
        throw new AppError("Code et nom requis", StatusCodes.BAD_REQUEST);
    const existing = await prisma.country.findUnique({ where: { code: code.toUpperCase() } });
    if (existing)
        throw new AppError("Ce pays existe déjà", StatusCodes.CONFLICT);
...
export const updateCountry = controllerWrapper(async (req, res) => {
...
export const deleteCountry = controllerWrapper(async (req, res) => {
    const code = String(req.params.code).toUpperCase();
    const country = await prisma.country.findUnique({ where: { code } });
    if (!country)
        throw new AppError("Pays introuvable", StatusCodes.NOT_FOUND);
