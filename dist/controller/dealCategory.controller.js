...
export const getAllDealCategories = controllerWrapper(async (_req, res) => {
...
export const createDealCategory = controllerWrapper(async (req, res) => {
...
export const updateDealCategory = controllerWrapper(async (req, res) => {
...
export const deleteDealCategory = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const existing = await prisma.dealCategory.findUnique({ where: { id } });
    if (!existing)
        throw new AppError('Catégorie introuvable', StatusCodes.NOT_FOUND);
...
//# sourceMappingURL=dealCategory.controller.js.map