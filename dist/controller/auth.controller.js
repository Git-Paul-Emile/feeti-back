...
export const register = controllerWrapper(async (req, res) => {
    ...
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Inscription réussie", data: { user, accessToken } }));
});
export const login = controllerWrapper(async (req, res) => {
    const { user, accessToken, refreshToken } = await authService.login(req.body);
    res.cookie(REFRESH_COOKIE, refreshToken, cookieOptions);
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Connexion réussie", data: { user, accessToken } }));
});
...
export const me = controllerWrapper(async (req, res) => {
    const user = await authService.getMe(req.user.userId);
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Profil récupéré", data: user }));
});
export const updateProfile = controllerWrapper(async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Profil mis à jour", data: user }));
});
export const changePassword = controllerWrapper(async (req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Mot de passe modifié avec succès" }));
});
...
export const refresh = controllerWrapper(async (req, res) => {
    ...
    if (!token) {
        res.status(StatusCodes.UNAUTHORIZED).json(jsonResponse({ status: "unauthorized", message: "Token de rafraîchissement manquant" }));
        return;
    }
    const payload = verifyRefreshToken(token);
    const user = await authService.getMe(payload.userId);
    const accessToken = generateToken({ userId: user.id, role: user.role });
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Token rafraîchi", data: { accessToken } }));
});
