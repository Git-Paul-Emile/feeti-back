...
export const getAllCountriesAdmin = controllerWrapper(async (_req, res) => {
    const countries = await prisma.country.findMany({ orderBy: { name: "asc" } });
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Pays récupérés", data: countries }));
});
// GET /api/admin/tickets/stats
export const getTicketsStats = controllerWrapper(async (_req, res) => {
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Statistiques billets", data: { totalSold, totalUsed, totalRefunded } }));
});
// GET /api/admin/tickets
export const getAdminTickets = controllerWrapper(async (req, res) => {
...
// GET /api/admin/users
export const getAllUsers = controllerWrapper(async (_req, res) => {
...
export const updateUserRole = controllerWrapper(async (req, res) => {
...
export const deleteUser = controllerWrapper(async (req, res) => {
    ...
    if (id === actorId)
        throw new AppError("Vous ne pouvez pas supprimer votre propre compte via l'admin", StatusCodes.BAD_REQUEST);
    const user = await prisma.user.findUnique({ where: { id } });
    if (!user)
        throw new AppError("Utilisateur introuvable", StatusCodes.NOT_FOUND);
    if (user.role === "super_admin")
        throw new AppError("Impossible de supprimer un super administrateur", StatusCodes.FORBIDDEN);
...
export const getAllAdminEvents = controllerWrapper(async (_req, res) => {
...
export const updateEventHighlight = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const { isFeatured, isFavorite } = req.body;
...
export const updateEventStatus = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
...
export const updateEventPromotion = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const { promotionType, promotionStatus, promotionStartDate, promotionEndDate } = req.body;
    ...
    if (promotionType && promotionStartDate && promotionEndDate && promotionStatus === "active") {
        ...
        if (rule) {
            ...
            if (days < rule.min || days > rule.max) {
                throw new AppError(`Durée invalide pour Pack ${promotionType} : ${rule.min}–${rule.max} jours requis (${days} jours saisis)`, StatusCodes.BAD_REQUEST);
            }
        }
    }
    const event = await prisma.event.findUnique({ where: { id } });
    ...
    if (promotionType && promotionStatus === "active") {
        const LIMITS = { OR: 2, ARGENT: 5, BRONZE: 10, LITE: Infinity };
        const now = new Date();
        ...
        if (activeCount >= typeLimit) {
            throw new AppError(`Limite atteinte : maximum ${typeLimit} événements Pack ${promotionType} simultanément`, StatusCodes.CONFLICT);
        }
    }
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Promotion mise à jour", data: updated }));
});
// GET /api/admin/events/promotion-slots
// Retourne les slots disponibles par type de pack
export const getPromotionSlots = controllerWrapper(async (_req, res) => {
    const LIMITS = { OR: 2, ARGENT: 5, BRONZE: 10, LITE: 9999 };
    const now = new Date();
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Slots récupérés", data: result }));
});
// GET /api/admin/leisure/promotion-slots
// Retourne les slots disponibles par type de pack loisir
export const getLeisurePromotionSlots = controllerWrapper(async (_req, res) => {
    const LEISURE_PACK_LIMITS = {
        CAMPAGNE_PREMIUM: 3,
        BOOST: 5,
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Slots loisirs récupérés", data: result }));
});
// PATCH /api/admin/leisure/:id/promotion
// Attribue une offre annuaire et/ou un pack ponctuel à un établissement loisir
export const updateLeisurePromotion = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const { leisureOfferType, leisurePackType, leisurePackStatus, leisurePackStartDate, leisurePackEndDate, } = req.body;
    ...
    if (leisurePackType && leisurePackStartDate && leisurePackEndDate && leisurePackStatus === "active") {
        ...
        const days = Math.round((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
        if (days !== 7) {
            throw new AppError(`Durée invalide pour Pack ${leisurePackType} : exactement 7 jours requis (${days} jours saisis)`, StatusCodes.BAD_REQUEST);
        }
    }
    ...
    if (leisurePackType && leisurePackStatus === "active") {
        ...
        if (activeCount >= packLimit) {
            throw new AppError(`Limite atteinte : maximum ${packLimit} établissements Pack ${leisurePackType} simultanément`, StatusCodes.CONFLICT);
        }
    }
    const item = await prisma.leisureItem.findUnique({ where: { id } });
    if (!item)
        throw new AppError("Établissement introuvable", StatusCodes.NOT_FOUND);
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Promotion loisir mise à jour", data: updated }));
});
