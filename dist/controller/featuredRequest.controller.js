...
export const submitFeaturedRequest = controllerWrapper(async (req, res) => {
    ...
    if (existing)
        throw new AppError("Une demande est déjà en attente pour cet événement", StatusCodes.CONFLICT);
    ...
    res.status(StatusCodes.CREATED).json(jsonResponse({ status: "success", message: "Demande de mise en avant envoyée", data: request }));
});
// GET /api/events/my-featured-requests
// Organisateur consulte l'état de ses demandes
export const getMyFeaturedRequests = controllerWrapper(async (req, res) => {
    const organizerId = req.user.userId;
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Demandes récupérées", data: requests }));
});
// GET /api/admin/featured-requests
// Admin liste toutes les demandes
export const getAllFeaturedRequests = controllerWrapper(async (_req, res) => {
    const requests = await prisma.featuredRequest.findMany({
        include: {
            event: {
                select: {
...
export const approveFeaturedRequest = controllerWrapper(async (req, res) => {
    ...
    if (!request)
        throw new AppError("Demande introuvable", StatusCodes.NOT_FOUND);
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Demande approuvée — événement mis en avant" }));
});
// PATCH /api/admin/featured-requests/:id/reject
// Admin rejette la demande
export const rejectFeaturedRequest = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const { adminNote } = req.body;
    ...
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Demande rejetée" }));
});
//# sourceMappingURL=featuredRequest.controller.js.map