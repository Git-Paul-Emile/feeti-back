...
import { createZoneSchema, updateZoneSchema, createCategorySchema, updateCategorySchema, setAccessRightsSchema, generateBadgeSchema, scanSchema, syncOfflineSchema, reportSuspectSchema, } from "../validators/access.validator.js";
const p = (v) => String(v);
// ─── Zones ────────────────────────────────────────────────────────────
...
export const applyDefaultZones = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const getZones = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
// ─── Catégories ───────────────────────────────────────────────────────
export const createCategory = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const applyDefaultCategories = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const updateCategory = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
...
export const deleteCategory = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const categoryId = p(req.params.categoryId);
    const { userId, role } = req.user;
...
// ─── Droits d'accès ───────────────────────────────────────────────────
export const setAccessRights = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
    const { rights } = setAccessRightsSchema.parse(req.body);
    const result = await accessService.setAccessRights(eventId, userId, role, rights);
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: "Droits d'accès mis à jour", data: result }));
});
export const applyDefaultMatrix = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const getAccessMatrix = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
// ─── Badges ───────────────────────────────────────────────────────────
export const generateBadge = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const getBadges = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const sendBadge = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
...
export const revokeBadge = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const badgeId = p(req.params.badgeId);
    const { userId, role } = req.user;
...
export const regenerateBadge = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const badgeId = p(req.params.badgeId);
    const { userId, role } = req.user;
...
// ─── Scan ─────────────────────────────────────────────────────────────
export const scanBadge = controllerWrapper(async (req, res) => {
...
export const syncOffline = controllerWrapper(async (req, res) => {
    const agentId = req.user.userId;
...
// ─── Tracking & Reporting ─────────────────────────────────────────────
export const getTracking = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const getAccessLogs = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const getRefusedAttempts = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const exportCsv = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
...
export const getCurrentQr = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const badgeId = p(req.params.badgeId);
    const { userId, role } = req.user;
...
// ─── Signalement ──────────────────────────────────────────────────────
export const reportSuspect = controllerWrapper(async (req, res) => {
    const badgeId = p(req.params.badgeId);
    const agentId = req.user.userId;
...
export const getSuspectReports = controllerWrapper(async (req, res) => {
    const eventId = p(req.params.eventId);
    const { userId, role } = req.user;
