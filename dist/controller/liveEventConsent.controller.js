...
import { liveEventConsentService } from "../services/liveEventConsent.service.js";
import { AppError } from "../utils/AppError.js";
const LIVE_EVENT_TYPES = ["STREAMING_LIVE", "MIXTE"];
/**
 * POST /api/live-event-consent/request-forms
 * Demande l'envoi des formulaires PDF pour un événement live/mixte.
 */
export async function requestLiveForms(req, res, next) {
    try {
        ...
        if (!LIVE_EVENT_TYPES.includes(eventType)) {
            throw new AppError(`eventType doit être l'un de : ${LIVE_EVENT_TYPES.join(", ")}`, StatusCodes.BAD_REQUEST);
        }
...
export async function recordConsent(req, res, next) {
    try {
        ...
        if (consentGiven !== true) {
            throw new AppError("Le consentement est obligatoire pour les événements en Streaming Live ou Mixte.", StatusCodes.BAD_REQUEST);
        }
...
//# sourceMappingURL=liveEventConsent.controller.js.map