...
import { leisureRepository } from '../repositories/leisure.repository.js';
// ── Public ────────────────────────────────────────────────────────────────────
// GET /api/leisure/categories
export const getAllLeisureCategories = controllerWrapper(async (_req, res) => {
...
// GET /api/leisure
export const getAllLeisureItems = controllerWrapper(async (req, res) => {
    const { categorySlug, countryCode, search, lat, lng } = req.query;
    const filters = {
        categorySlug,
        countryCode,
        search,
...
// GET /api/leisure/:id
export const getLeisureItemById = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const item = await leisureRepository.findById(id);
    if (!item)
        throw new AppError('Loisir introuvable', StatusCodes.NOT_FOUND);
...
export const getMyLeisureFavorites = controllerWrapper(async (req, res) => {
...
export const checkLeisureFavorite = controllerWrapper(async (req, res) => {
    const userId = req.user.userId;
...
export const toggleLeisureFavorite = controllerWrapper(async (req, res) => {
    const userId = req.user.userId;
    const leisureItemId = String(req.params.id);
...
export const getAllLeisureAdmin = controllerWrapper(async (_req, res) => {
    const items = await leisureRepository.findAllAdmin();
    res.status(StatusCodes.OK).json(jsonResponse({ status: 'success', message: 'Loisirs récupérés', data: items }));
});
// POST /api/admin/leisure
export const createLeisureItem = controllerWrapper(async (req, res) => {
    const { name, description, categorySlug, location, address, phone, website, priceRange, openingHours, image, rating, reviewCount, features, tags, status, countryCode, isFeatured, } = req.body;
    ...
    if (!catExists)
        throw new AppError('Catégorie de loisir introuvable', StatusCodes.BAD_REQUEST);
...
export const updateLeisureItem = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    ...
    if (priceRange !== undefined)
        data.priceRange = String(priceRange);
    if (openingHours !== undefined)
        data.openingHours = String(openingHours);
    if (image !== undefined)
...
export const deleteLeisureItem = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const existing = await leisureRepository.findById(id);
    if (!existing)
        throw new AppError('Loisir introuvable', StatusCodes.NOT_FOUND);
...
export const createLeisureCategory = controllerWrapper(async (req, res) => {
    ...
    if (existing)
        throw new AppError('Une catégorie avec ce nom ou slug existe déjà', StatusCodes.BAD_REQUEST);
...
export const updateLeisureCategory = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const { name, slug } = req.body;
...
export const deleteLeisureCategory = controllerWrapper(async (req, res) => {
    const id = String(req.params.id);
    const existing = await prisma.leisureCategory.findUnique({ where: { id } });
    if (!existing)
        throw new AppError('Catégorie introuvable', StatusCodes.NOT_FOUND);
...
//# sourceMappingURL=leisure.controller.js.map