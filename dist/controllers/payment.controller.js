...
export const createStripeIntent = controllerWrapper(async (req, res) => {
...
export const initializeMobileMoney = controllerWrapper(async (req, res) => {
    const { amount, currency = "FCFA", phone, provider, metadata } = req.body;
    if (!phone)
        throw new AppError("Numéro de téléphone requis", StatusCodes.BAD_REQUEST);
...
// ─── Mobile Money : statut ───────────────────────────────────────────
export const getMobileMoneyStatus = controllerWrapper(async (req, res) => {
    const txId = String(req.params["txId"]);
    const result = await paymentService.getMobileMoneyStatus(txId);
    res.status(StatusCodes.OK).json(jsonResponse({ status: "success", message: result.message, data: { status: result.status } }));
});
// ─── Paystack : initialiser ──────────────────────────────────────────
export const initializePaystack = controllerWrapper(async (req, res) => {
    const { amount, currency = "FCFA", email, metadata } = req.body;
    if (!email)
        throw new AppError("Email requis pour Paystack", StatusCodes.BAD_REQUEST);
...
export const confirmPaymentAndPurchase = controllerWrapper(async (req, res) => {
    const userId = req.user.userId;
    const { eventId, holderName, holderEmail, holderPhone, items, // [{ category, quantity, price }]
    delivery, // optionnel
    ...
    // Validation basique
    if (!eventId)
        throw new AppError("eventId requis", StatusCodes.BAD_REQUEST);
    if (!holderName || !holderEmail)
        throw new AppError("Nom et email du porteur requis", StatusCodes.BAD_REQUEST);
    ...
    const ticketsWithQR = await Promise.all(purchaseResult.tickets.map(async (ticket) => {
        let qrDataUrl;
        try {
