...
export const TAUX_TVA_BP = 1800; // 18.00%
export const TAUX_COMMISSION_BP = 1000; // 10.00%
export const BP_DIVISOR = 10000; // 10 000 basis points = 100%
...
export function arrondiHalfUp(valeur) {
    if (!isFinite(valeur)) {
...
export function validerMontantCentimes(montant) {
    const errors = [];
    ...
    if (!Number.isInteger(montant)) {
        errors.push(`Le montant doit être un entier (centimes entiers), reçu: ${montant}. ` +
    ...
    if (montant > MAX_AMOUNT_CENTIMES) {
        errors.push(`Dépassement de limite: ${montant} centimes > ${MAX_AMOUNT_CENTIMES} ` +
 ...
 */
export function calculerRepartitionFinanciere(prixTTCCentimes, tauxTVA_bp = TAUX_TVA_BP, tauxCommission_bp = TAUX_COMMISSION_BP) {
    // Validation stricte
    const validation = validerMontantCentimes(prixTTCCentimes);
    if (!validation.valid) {
        throw new Error(`Montant TTC invalide: ${validation.errors.join("; ")}`);
    }
    ...
    if (prixHT + tva !== prixTTC) {
        throw new Error(`ERREUR CRITIQUE — décomposition fiscale: HT(${prixHT}) + TVA(${tva}) = ${prixHT + tva} ≠ TTC(${prixTTC}). ` +
    ...
    if (distribution !== prixTTC) {
        throw new Error(`ERREUR CRITIQUE — distribution: Net(${netOrganisateur}) + TVA(${tva}) + Commission(${commission}) = ${distribution} ≠ TTC(${prixTTC}). ` +
            `Écart: ${distribution - prixTTC} centimes.`);
    }
    if (netOrganisateur < 0) {
        throw new Error(`Net organisateur négatif (${netOrganisateur}): commission (${commission}) > (TTC - TVA) (${prixTTC - tva})`);
    }
 ...
 */
export function verifierCohérenceRepartition(repartition) {
...
export function xofToCentimes(montantXOF) {
...
export function eurToCentimes(montantEUR) {
...
export function centimesToEUR(centimes) {
    if (!Number.isInteger(centimes)) {
        throw new Error(`Les centimes doivent être un entier, reçu: ${centimes}`);
    }
    return centimes / 100;
 ...
 */
export function formatMontant(centimes, devise = "XOF") {
    if (!Number.isInteger(centimes)) {
...
export function formatCurrency(centimes, devise = "XOF") {
    return {
        montant: centimes,
...
export function bpToPourcent(bp) {
    return `${(bp / 100).toFixed(2)}%`;
}
/** Convertit un pourcentage en basis points */
export function pourcentToBP(pourcent) {
 ...
 */
export function genererChecksumRepartition(repartition) {
 ...
 */
export function genererChecksum(data) {
...
export function agregerRepartitions(repartitions) {
    return repartitions.reduce((acc, r) => ({
...
export function extractTVA(prixTTC, tauxTVA_bp = TAUX_TVA_BP) {
    const tvaDivisor = BP_DIVISOR + tauxTVA_bp;
    const prixHT = arrondiHalfUp((prixTTC * BP_DIVISOR) / tvaDivisor);
