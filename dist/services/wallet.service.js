...
export const walletService = {
    async getOrCreateWallet(organizerId, devise = "XOF") {
        return walletRepository.upsertForOrganizer(organizerId, devise);
    },
    async getWalletByOrganizerId(organizerId) {
    ...
    async getWalletSummary(organizerId) {
    ...
    async getLedger(organizerId, options) {
        const wallet = await walletRepository.findByOrganizerId(organizerId);
        if (!wallet) {
     ...
     */
    async verifierIntegrite(organizerId) {
        const wallet = await walletRepository.findByOrganizerId(organizerId);
        if (!wallet) {
            throw new AppError("Wallet introuvable", StatusCodes.NOT_FOUND);
        }
     ...
     */
    async ajustementAdmin(organizerId, montantCentimes, type, motif, adminId, adminRole) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const balanceAfter = type === "credit"
                ...
                : balanceBefore - montantCentimes;
            if (balanceAfter < 0) {
                throw new AppError(`Ajustement débit impossible: solde résultant négatif (${balanceAfter} centimes)`, StatusCodes.BAD_REQUEST);
            }
