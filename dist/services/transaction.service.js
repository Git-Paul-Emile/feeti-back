...
import { calculerRepartitionFinanciere, genererChecksum, pourcentToBP, } from "../utils/financial.js";
import { transactionRepository } from "../repositories/transaction.repository.js";
import { walletService } from "./wallet.service.js";
import { auditService } from "./audit.service.js";
// ─── Lecture des taux configurés en base ─────────────────────────────
export async function lireTauxConfigures() {
...
function assertTransitionAllowed(from, to) {
    ...
    if (!allowed.includes(to)) {
        throw new AppError(`Transition d'état invalide: ${from} → ${to}. ` +
            `Transitions autorisées depuis '${from}': [${allowed.join(", ") || "aucune"}]`, StatusCodes.UNPROCESSABLE_ENTITY);
    }
}
// ─── Service ──────────────────────────────────────────────────────────
export const transactionService = {
    ...
    async creerTransaction(data) {
        ...
        if (existingTicketTx) {
            throw new AppError(`Une transaction existe déjà pour le billet ${data.ticketId}`, StatusCodes.CONFLICT);
        }
        ...
        const repartition = calculerRepartitionFinanciere(data.prixTTCCentimes, tauxTVA_bp, tauxCommission_bp);
     ...
     */
    async changerStatut(transactionId, newStatus, actor, options) {
    ...
    async _appliquerEffetWallet(tx, transaction, oldStatus, newStatus, options) {
    ...
    async listerTransactions(filters, userId, userRole) {
        const isAdmin = ["admin", "super_admin"].includes(userRole);
        if (!isAdmin) {
    ...
    async confirmerPaiement(transactionId, providerTransactionId, actor) {
    ...
    async completerTransaction(transactionId, actor) {
        return transactionService.changerStatut(transactionId, "completed", actor, {
            reason: "Transaction finalisée",
        });
    },
    async initierRemboursement(transactionId, montantRemboursement, raison, actor) {
        const transaction = await transactionRepository.findById(transactionId);
        if (!transaction)
            throw new AppError("Transaction introuvable", StatusCodes.NOT_FOUND);
        if (montantRemboursement > transaction.montantTTC) {
            throw new AppError(`Montant du remboursement (${montantRemboursement}) > TTC (${transaction.montantTTC})`, StatusCodes.BAD_REQUEST);
        }
