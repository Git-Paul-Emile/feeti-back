...
function generateQRData(ticketId, orderId, eventId) {
...
export const ticketService = {
    async purchaseTickets(data) {
        ...
        if (event.status !== "published")
            throw new AppError("Cet événement n'est pas disponible à la vente", StatusCodes.BAD_REQUEST);
        ...
        if (data.delivery?.method === "physical") {
            if (!data.delivery.zoneId)
                throw new AppError("Zone de livraison obligatoire", StatusCodes.BAD_REQUEST);
            ...
            if (!zone)
                throw new AppError("Zone de livraison introuvable", StatusCodes.NOT_FOUND);
    ...
    async getTicketById(id, userId, role) {
        const ticket = await ticketRepository.findById(id);
        if (!ticket)
            throw new AppError("Billet introuvable", StatusCodes.NOT_FOUND);
    ...
    async verifyTicket(qrData, organizerId, role) {
    ...
    async getEventTickets(eventId, organizerId, role) {
