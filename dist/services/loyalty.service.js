...
const DEFAULT_PARTNER_DISCOUNTS = {
    Mobembo: "5%",
    Elengi: "10%",
    Momi: "20%",
    Mwana: "30%",
...
export const loyaltyService = {
    ...
    async onTicketPurchase(userId, amountFCFA, ticketId) {
        ...
        await loyaltyRepository.updatePoints(profile.id, totalPoints, "ticket_purchase", `Achat billet — ${amountFCFA.toLocaleString("fr-FR")} FCFA`, ticketId, "ticket");
    ...
    async onEventAttendance(userId, eventId) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        ...
        await loyaltyRepository.updatePoints(profile.id, points, "event_attendance", `Présence à l'événement`, eventId, "event");
        ...
        if (!profile.referralFirstEventBonusPaid && profile.referredById) {
            ...
            await loyaltyRepository.updatePoints(profile.referredById, POINTS_REFERRAL_FIRST_EVENT, "referral_first_event", `Bonus parrainage — premier événement de votre filleul`, profile.userId, "user");
        }
    },
    async onEventShare(userId, eventId) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        ...
        await loyaltyRepository.updatePoints(profile.id, POINTS_SHARE, "event_share", `Partage d'événement`, eventId, "event");
    ...
    async applyReferralCode(newUserId, referralCode) {
        ...
        if (!referrerProfile)
            throw new AppError("Code de parrainage invalide", StatusCodes.BAD_REQUEST);
        ...
        await loyaltyRepository.updatePoints(referrerProfile.id, POINTS_REFERRAL_SIGNUP, "referral_signup", `Parrainage — inscription de votre filleul`, newUserId, "user");
    ...
    //
    async simulatePointsPayment(userId, amountFCFA) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
    ...
    async applyPointsPayment(userId, pointsToUse, ticketId, amountFCFA) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        if (pointsToUse <= 0)
        ...
        await loyaltyRepository.updatePoints(profile.id, -pointsToUse, "reward_redemption", `Réduction billet — ${discountFCFA.toLocaleString("fr-FR")} FCFA déduits`, ticketId, "ticket");
    ...
    async getVipBenefits(userId) {
        ...
        const thresholds = { Mobembo: 0, Elengi: 1000, Momi: 2000, Mwana: 3000, Boboto: 4000 };
    ...
    async getAmbassadorStatus(userId) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        const badges = await loyaltyRepository.getBadgesForUser(userId);
        const { prisma } = await import("../config/database.js");
        const [postCount, referralCount, eventsAttended, vipLogs,] = await Promise.all([
    ...
    async redeemReward(userId, rewardId) {
        ...
        if (!reward)
            throw new AppError("Récompense introuvable", StatusCodes.NOT_FOUND);
        if (!reward.isActive)
            throw new AppError("Cette récompense n'est plus disponible", StatusCodes.BAD_REQUEST);
        if (reward.stock <= 0)
            throw new AppError("Stock épuisé", StatusCodes.BAD_REQUEST);
        ...
        await loyaltyRepository.updatePoints(profile.id, -reward.points, "reward_redemption", `Échange récompense : ${reward.title}`, rewardId, "reward");
    ...
    async getLeaderboard(userId) {
        ...
        if (!userInTop) {
            const { prisma } = await import("../config/database.js");
            const rank = (await prisma.loyaltyProfile.count({ where: { points: { gt: profile.points } } })) + 1;
            formatted.push({
                rank,
    ...
    async recordPartnerSpending(userId, partnerId, amountFCFA, description) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        const partner = await loyaltyRepository.getPartnerById(partnerId);
        if (!partner || !partner.isActive)
            throw new AppError("Partenaire introuvable ou inactif", StatusCodes.NOT_FOUND);
        ...
        if (totalPoints > 0) {
            await loyaltyRepository.updatePoints(profile.id, totalPoints, "partner_spending", `Dépense chez ${partner.name} — ${amountFCFA.toLocaleString("fr-FR")} FCFA`, partnerId, "partner");
        }
        await checkAndProgressMissions(profile.id, "partner_spending");
        return { pointsEarned: totalPoints, partner: partner.name };
    },
    async getPartnerDiscountForUser(userId, partnerId) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        const partner = await loyaltyRepository.getPartnerById(partnerId);
        if (!partner)
            throw new AppError("Partenaire introuvable", StatusCodes.NOT_FOUND);
        const byLevel = partner.discountByLevel;
    ...
    async createCommunityPost(userId, data) {
        ...
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
        await loyaltyRepository.updatePoints(profile.id, POINTS_COMMUNITY_POST, "community_post", `Publication communauté : ${data.title}`, post.id, "post");
        await checkAndProgressMissions(profile.id, "community_post");
        await checkContentCreatorBadge(userId);
        return post;
    },
    async engageCommunityPost(userId, postId, type, content) {
        const post = await loyaltyRepository.getCommunityPostById(postId);
        if (!post || post.status !== "approved")
            throw new AppError("Publication introuvable", StatusCodes.NOT_FOUND);
        const alreadyEngaged = await loyaltyRepository.hasEngaged(userId, postId, type);
        if (alreadyEngaged && type === "like")
            throw new AppError("Déjà liké", StatusCodes.CONFLICT);
        ...
        await loyaltyRepository.updatePoints(profile.id, pts, "community_engagement", `${type === "like" ? "Like" : "Commentaire"} sur publication`, postId, "post");
        return { pointsEarned: pts };
    },
    async moderateCommunityPost(postId, status) {
    ...
    // ─── VIP ────────────────────────────────────────────────────────────────────
    async grantVipAccess(userId, eventId, accessType, ticketId) {
        const profile = await loyaltyRepository.getOrCreateProfile(userId);
    ...
    async getAdminBadges(filters) {
        return loyaltyRepository.getAllBadges(filters);
    },
    async getAdminCommunityPosts(filters) {
        return loyaltyRepository.getCommunityPosts(filters);
    },
    async adminUpdateCommunityPost(postId, data) {
        return loyaltyRepository.updateCommunityPost(postId, data);
    },
    async adminDeleteCommunityPost(postId) {
...
async function getEffectiveMultiplier(level, actionType) {
...
async function updateTotalSpent(profileId, amountFCFA) {
    const { prisma } = await import("../config/database.js");
    await prisma.loyaltyProfile.update({
        where: { id: profileId },
...
async function checkAndProgressMissions(profileId, actionType) {
    const missions = await loyaltyRepository.getMissions(true);
    for (const mission of missions) {
        if (mission.actionType !== actionType)
            continue;
        ...
        if (prog.completed && !prog.rewardPaid) {
            ...
            await loyaltyRepository.updatePoints(profileId, mission.points, "mission_complete", `Mission accomplie : ${mission.title}`, mission.id, "mission");
        }
    }
}
// Bonus activité : paliers d'événements dans le mois en cours
async function checkActivityBonus(profileId, level) {
    const { prisma } = await import("../config/database.js");
...
async function checkReferralBadge(userId, referralCount) {
    const thresholds = [
...
async function checkContentCreatorBadge(userId) {
    const { prisma } = await import("../config/database.js");
