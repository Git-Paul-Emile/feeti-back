...
export const reportingService = {
    ...
    async getDashboardOrganisateur(organizerId, periode) {
    ...
    // ─── Dashboard admin ────────────────────────────────────────────────
    async getDashboardAdmin(periode) {
        const { dateFrom, dateTo } = periode;
        const [fluxAggregat, payoutsCompleted, transactionsParStatut, topOrganisateurRaw, litiges,] = await Promise.all([
            prisma.transaction.aggregate({
                where: {
                    status: { in: ["completed", "paid"] },
                    createdAt: { gte: dateFrom, lte: dateTo },
        ...
        const alertes = [];
        if (litiges > 10)
            alertes.push(`⚠️ ${litiges} transactions en litige`);
    ...
    async getRapportTVA(periode, organizerId) {
        const { dateFrom, dateTo } = periode;
    ...
    async exportTransactionsCSV(filters, devise = "XOF") {
    ...
    async getTendancesVentes(organizerId, mois = 6) {
    ...
    async getRapportRemboursements(organizerId, periode) {
        ...
        const totalRembourse = remboursements.reduce((sum, t) => sum + (t.refundAmount ?? t.montantTTC), 0);
        return {
            remboursements,
