...
function isFeetiPlayLiveId(id) {
    return id.startsWith(FEETIPLAY_LIVE_ID_PREFIX);
}
function mapSyncedLiveEvent(event) {
    return {
        id: event.id,
        title: event.title,
...
export const eventService = {
    ...
    async getAllEvents(countryCode, featuredOnly, interests) {
    ...
    async deleteEvent(eventId, organizerId, role) {
        ...
        if (!isAdmin) {
            ...
            if (ticketCount > 0) {
                throw new AppError(`Impossible de supprimer : ${ticketCount} billet(s) déjà vendu(s) pour cet événement`, StatusCodes.CONFLICT);
            }
        }
        return eventRepository.delete(eventId);
    },
    async toggleSalesBlocked(eventId, organizerId, role) {
        if (isFeetiPlayLiveId(eventId)) {
            const event = await feetiPlaySyncService.getLiveEventById(eventId);
            const isAdmin = role === "admin" || role === "super_admin";
            if (!isAdmin && event.organizerId !== organizerId) {
        ...
        const newBlocked = !event.salesBlocked;
    ...
    async updateEvent(eventId, organizerId, data, role) {
        if (isFeetiPlayLiveId(eventId)) {
            const event = await feetiPlaySyncService.getLiveEventById(eventId);
            const isAdmin = role === "admin" || role === "super_admin";
            if (!isAdmin && event.organizerId !== organizerId) {
        ...
        if (!isAdmin) {
            const ticketCount = await ticketRepository.countByEvent(eventId);
            if (ticketCount > 0) {
                throw new AppError(`Impossible de modifier : ${ticketCount} billet(s) déjà vendu(s) pour cet événement`, StatusCodes.CONFLICT);
            }
        }
        if (data.isLive === true) {
            const organizer = await prisma.user.findUnique({
